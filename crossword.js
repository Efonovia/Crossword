...
const downSection = document.querySelector('.down-clues')
const acrossSection = document.querySelector('.across-clues')
let gridIndex = 0

console.log(data.clues.across.length)
console.log(data.clues.down.length)
for(let i = 0; i < data.size.rows; i++) {
    const row = table.insertRow()
    for(let j = 0; j < data.size.cols; j++) {
        const cell = row.insertCell()
        const gridHolder = document.createElement("div")
        if(data.grid[gridIndex] !== ".") {
            gridHolder.className = "grid-holder"
            cell.style.background = "#7e7e7e"
        } else {
            cell.style.background = "black"
            cell.style.cursor = "auto"
        }

        const gridNum = document.createElement("div")
        gridNum.className = "grid-num"
        if(data.gridnums[gridIndex] !== 0) {
            gridNum.textContent = data.gridnums[gridIndex]
        } else {
            gridNum.textContent = data.gridnums[gridIndex]
            gridNum.style.color = "#7e7e7e"
        }
        
        const gridText = document.createElement("div")
        if(data.grid[gridIndex] !== ".") {
            gridText.className = "grid-text"
            gridText.textContent = data.grid[gridIndex]
        } 

        if(data.grid[gridIndex] === ".") {
            gridNum.textContent = data.gridnums[gridIndex]
            gridNum.style.color = "black"
        }
        gridIndex += 1

        gridHolder.appendChild(gridNum)
        gridHolder.appendChild(gridText)
        cell.appendChild(gridHolder)
    }
}
...
for(let i = 0; i < data.clues.across.length; i++) {
    let acrossClue = document.createElement('div')
    acrossClue.textContent = data.clues.across[i]
    acrossSection.appendChild(acrossClue)
}

for(let i = 0; i < data.clues.down.length; i++) {
    let downClue = document.createElement('div')
    downClue.textContent = data.clues.down[i]
    downSection.appendChild(downClue)
}